...
public final class Game
{
	...
	public EnumMap<GHOST,Integer> rewards = new EnumMap<GHOST,Integer>(GHOST.class);
	public EnumMap<GHOST,Integer> coms_rewards = new EnumMap<GHOST,Integer>(GHOST.class);
...
*/	
	...
	private void _init(int initialMaze)
	{
		...
		_initGhosts();
		//System.out.println(currentMaze.lairNodeIndex);
		currentMaze.lairNodeIndex = 100;
	...
	private void _initGhosts()
	{
		...
		for(GHOST ghostType : GHOST.values()){
			...
			rewards.put(ghostType, 0);
			coms_rewards.put(ghostType, 0);
		}
	}
	...
	{		
		...
		for(GHOST ghost_key : GHOST.values())
		{
			Ghost ghost=  ghosts.get(ghost_key);
			int distance=getShortestPathDistance(pacman.currentNodeIndex, ghost.currentNodeIndex);
			
			
			for(GHOST g_ : GHOST.values())
			{
				int gdistance=getShortestPathDistance(ghosts.get(g_).currentNodeIndex, ghost.currentNodeIndex);
				//System.out.println(distance+" "+5*constants.EAT_DISTANCE);
				if(distance<=10*constants.EAT_DISTANCE && distance!=-1) //eaten
				{
					
					rewards.put(ghost_key, -1);
					rewards.put(g_, -1);
				}
			}
			
			if(distance<=constants.EAT_DISTANCE && distance!=-1) //eaten
			{
